    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="max-w-2xl mx-auto px-6 pt-20 pb-4">
        <motion.div initial="hidden" animate="show" variants={stagger}>
          <motion.p variants={fadeUp} className="text-xs font-medium text-muted-foreground uppercase tracking-widest mb-4">Cookies</motion.p>
      ...
      <section className="max-w-2xl mx-auto px-6 pb-20">
        <motion.div initial="hidden" animate="show" variants={stagger}>
          ...
          <motion.div variants={fadeUp} className="mb-10">
            <h2 className="text-lg font-semibold tracking-tight mb-4">Types of cookies we use</h2>
              ...
              {COOKIE_TYPES.map((ct, i) => (
                <div key={i} className="border border-border rounded-lg p-5 bg-card">
                  <div className="flex items-start justify-between gap-3 mb-2">
                    ...
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${ct.required ? "bg-accent/10 text-accent" : "bg-muted text-muted-foreground"}`}>
                      {ct.required ? "Required" : "Optional"}
                    </span>
                  </div>
                  <p className="text-sm text-muted-foreground mb-2">{ct.purpose}</p>
          ...
          <Section title="Managing cookies">
            ...
            <p>To manage cookies in your browser, visit the settings or preferences menu. Common locations:</p>
